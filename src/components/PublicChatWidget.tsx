...
export type PublicChatMessage = { id: string; role: "client" | "ai" | "system"; content: string };
...
export function PublicChatWidget({ organizationSlug, embedded = false }: { organizationSlug: string; embedded?: boolean }) {
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    try {
      ...
      if (data.status === "humano") {
        setHumanMode(true);
        setMessages((items) => [...items, { id: crypto.randomUUID(), role: "system", content: "Sua mensagem foi enviada. Um atendente humano assumiu esta conversa." }]);
  ...
  const panel = <div className={`${embedded ? "h-full min-h-[620px]" : "h-[620px] w-[min(390px,calc(100vw-24px))] shadow-2xl"} flex flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white`}>
    ...
    <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto bg-[#f7f9f8] p-4">{messages.length === 0 && <div className="rounded-2xl rounded-bl-md bg-white px-4 py-3 text-sm leading-6 text-slate-600 shadow-sm">Olá! 👋 Sou a recepcionista virtual. Posso tirar dúvidas, consultar horários e fazer seu agendamento.</div>}{messages.map((message) => <div key={message.id} className={`flex ${message.role === "client" ? "justify-end" : "justify-start"}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "client" ? "rounded-br-md bg-slate-950 text-white" : message.role === "ai" ? "rounded-bl-md bg-white text-slate-700 shadow-sm" : "border border-amber-200 bg-amber-50 text-amber-800"}`}>{message.content}</div></div>)}{sending && <div className="flex justify-start"><div className="rounded-2xl rounded-bl-md bg-white px-4 py-3 text-sm text-slate-400 shadow-sm">Digitando...</div></div>}</div>
    <form onSubmit={sendMessage} className="border-t border-slate-100 bg-white p-4"><div className="flex gap-2"><input value={text} onChange={(e) => setText(e.target.value)} placeholder="Digite sua mensagem..." className="h-11 flex-1 rounded-xl border border-slate-200 px-4 text-sm outline-none focus:border-emerald-500" /><button disabled={sending || !text.trim()} className="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-500 text-white disabled:opacity-40"><Send className="h-4 w-4" /></button></div>{publicToken && <button type="button" onClick={resetConversation} className="mt-2 text-xs font-semibold text-slate-400 hover:text-slate-700">Iniciar nova conversa</button>}</form>
  </div>;

...
const mins = (v: string) => { const [h, m] = hhmm(v).split(":").map(Number); return h * 60 + m; };
const weekday = (date: string, tz: string) => Number(formatInTimeZone(fromZonedTime(`${date}T12:00:00`, tz), tz, "i"));

type Service = { id: string; name: string; description: string | null; price_cents: number; duration_minutes: number; professional_id: string | null };

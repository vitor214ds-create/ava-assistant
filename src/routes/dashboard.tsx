  ...
  return <main className="min-h-screen bg-[#f7f9f8] text-slate-900">
    <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-slate-200 bg-white lg:block">
      ...
      <nav className="space-y-1 overflow-y-auto p-4 pb-24 text-sm font-semibold" style={{maxHeight:"calc(100vh - 72px)"}}>
        ...
        <NavLink to="/assinatura" icon={Crown} label="Assinatura" />
    ...
    <div className="lg:pl-64">
      <header className="sticky top-0 z-20 flex h-18 items-center justify-between border-b border-slate-200 bg-white/90 px-5 backdrop-blur-xl sm:px-8"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">RecepIA</p><p className="font-semibold text-slate-950">{companyName}</p></div><div className="flex items-center gap-3"><Link to="/conversas" aria-label="Abrir conversas" title="Abrir conversas" className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:text-emerald-600"><Bell className="h-4 w-4" /></Link><div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-950 text-sm font-bold text-white">{user?.email?.slice(0, 1).toUpperCase() ?? "U"}</div></div></header>

      <div className="border-b border-slate-200 bg-white lg:hidden"><nav className="flex gap-2 overflow-x-auto px-4 py-3 text-xs font-bold"><MobileNav to="/agenda" icon={CalendarCheck2} label="Agenda"/><MobileNav to="/clientes" icon={UsersRound} label="Clientes"/><MobileNav to="/conversas" icon={MessageCircleMore} label="Conversas"/><MobileNav to="/configuracao-ia" icon={Sparkles} label="IA"/><MobileNav to="/integracoes" icon={PlugZap} label="Integrações"/><MobileNav to="/configuracoes-empresa" icon={Building2} label="Empresa"/><MobileNav to="/ajuda" icon={CircleHelp} label="Ajuda"/>{isPlatformAdmin&&<MobileNav to="/admin" icon={ShieldCheck} label="Admin"/>}</nav></div>
      ...
      <div className="mx-auto max-w-7xl p-5 sm:p-8">
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_.6fr]">
          ...
          <section className="rounded-3xl bg-slate-950 p-6 text-white"><div className="flex items-center justify-between"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-500"><Bot className="h-5 w-5" /></div><span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-400">IA ativa</span></div><h2 className="mt-7 font-[Sora] text-xl font-semibold">IA trabalhando</h2><p className="mt-2 text-sm leading-6 text-slate-400">Sua recepcionista continua disponível para atender novos clientes e consultar horários.</p><div className="mt-7 space-y-4"><MiniStat label="Conversas atendidas hoje" value={stats.conversations} /><MiniStat label="Agendamentos realizados pela IA" value={stats.aiAppointments} accent /></div><Link to="/integracoes" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-emerald-400">Configurar WhatsApp<ArrowUpRight className="h-4 w-4" /></Link></section>
        </div>
      </div>
    </div>
  </main>;
...
function NavLink({ to, icon: Icon, label, active = false }: { to: AppRoute; icon: typeof CalendarDays; label: string; active?: boolean }) { return <Link to={to} className={`flex items-center gap-3 rounded-xl px-3 py-3 ${active ? "bg-emerald-50 text-emerald-700" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}><Icon className="h-4 w-4" />{label}</Link>; }
function MobileNav({to,icon:Icon,label}:{to:AppRoute;icon:typeof CalendarDays;label:string}){return <Link to={to} className="flex shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-slate-600"><Icon className="h-4 w-4 text-emerald-600"/>{label}</Link>}

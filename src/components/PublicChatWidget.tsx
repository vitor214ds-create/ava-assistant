...
export function PublicChatWidget({ organizationSlug, embedded = false }: { organizationSlug: string; embedded?: boolean }) {
  ...
  function resetConversation() {
    setPublicToken(null); setMessages([]); setHumanMode(false);
  ...
  const panel = <div className={`${embedded ? "h-full min-h-[620px]" : "h-[620px] w-[min(390px,calc(100vw-24px))] shadow-2xl"} flex flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white`}>
    ...
    <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto bg-[#f7f9f8] p-4">{messages.length === 0 && <div className="rounded-2xl rounded-bl-md bg-white px-4 py-3 text-sm leading-6 text-slate-600 shadow-sm">Olá! 👋 Como posso ajudar você hoje?</div>}{messages.map((message) => <div key={message.id} className={`flex ${message.role === "client" ? "justify-end" : "justify-start"}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "client" ? "rounded-br-md bg-slate-950 text-white" : message.role === "ai" ? "rounded-bl-md bg-white text-slate-700 shadow-sm" : "border border-amber-200 bg-amber-50 text-amber-800"}`}>{message.content}</div></div>)}{sending && <div className="flex justify-start"><div className="rounded-2xl rounded-bl-md bg-white px-4 py-3 text-sm text-slate-400 shadow-sm">Digitando...</div></div>}</div>

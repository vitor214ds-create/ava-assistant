...
import { ArrowLeft, Bot, CalendarCheck2, Check, Clock3, Crown, MessageCircleMore, ShieldCheck, Sparkles, UserRoundCog } from "lucide-react";
...
type Subscription = { id: string; plan_id: string | null; status: string; trial_ends_at: string; current_period_end: string | null };
...
function SubscriptionPage() {
  ...
  async function loadBilling() {
    ...
    let { data: subscriptionData } = await supabase.from("subscriptions").select("id,plan_id,status,trial_ends_at,current_period_end").eq("organization_id", organizationId).maybeSingle();
    if (!subscriptionData) {
      const basic = typedPlans.find((plan) => plan.slug === "basico") ?? typedPlans[0];
      const { data: created, error: createError } = await supabase.from("subscriptions").insert({ organization_id: organizationId, plan_id: basic.id, status: "trial" }).select("id,plan_id,status,trial_ends_at,current_period_end").single();
  ...
  return <main className="min-h-screen bg-[#f7f9f8] text-slate-900">
    ...
    <div className="mx-auto max-w-7xl p-5 sm:p-8"><div><p className="text-sm font-semibold text-emerald-600">Cobrança e limites</p><h1 className="mt-1 font-[Sora] text-3xl font-bold">Assinatura.</h1><p className="mt-2 text-sm text-slate-500">Acompanhe seu plano atual, período de teste e consumo mensal.</p></div>
      ...
      {loadingData ? <div className="mt-10 text-center text-sm text-slate-500">Carregando assinatura...</div> : <>
        ...
        <section className="mt-6"><div><h2 className="font-[Sora] text-2xl font-bold">Planos disponíveis</h2><p className="mt-1 text-sm text-slate-500">A troca de plano só será efetivada após confirmação do pagamento.</p></div><div className="mt-5 grid gap-5 lg:grid-cols-3">{plans.map((plan)=><PlanCard key={plan.id} plan={plan} current={plan.id===currentPlan?.id}/>)}</div></section>

        <section className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-5"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-amber-700"/><div><h3 className="font-semibold text-amber-900">Cobrança segura ainda não conectada</h3><p className="mt-1 text-sm leading-6 text-amber-800">Por segurança, os botões de troca de plano não alteram sua assinatura diretamente. O próximo passo é conectar um provedor de pagamentos e confirmar upgrades somente por webhook após pagamento aprovado.</p></div></div></section>
      </>}
    </div>
  </main>;
}
...
function PlanCard({plan,current}:{plan:Plan;current:boolean}){return <article className={`rounded-3xl border p-6 ${current?"border-emerald-500 bg-emerald-50":"border-slate-200 bg-white"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-emerald-600">{plan.slug==="premium"?"Operação completa":plan.slug==="profissional"?"Mais escolhido":"Essencial"}</p><h3 className="mt-1 font-[Sora] text-xl font-bold">{plan.name}</h3></div>{current && <span className="rounded-full bg-emerald-500 px-3 py-1 text-xs font-bold text-white">Atual</span>}</div><p className="mt-5"><span className="font-[Sora] text-3xl font-bold">R$ {(plan.price_cents/100).toLocaleString("pt-BR",{minimumFractionDigits:2})}</span><span className="text-sm text-slate-500">/mês</span></p><ul className="mt-6 space-y-3">{(Array.isArray(plan.features)?plan.features:[]).map((feature)=><li key={feature} className="flex items-start gap-2 text-sm text-slate-600"><Check className="mt-0.5 h-4 w-4 shrink-0 text-emerald-500"/>{feature}</li>)}</ul><button disabled className={`mt-7 h-11 w-full rounded-xl text-sm font-bold ${current?"bg-emerald-100 text-emerald-700":"bg-slate-100 text-slate-400"}`}>{current?"Plano atual":"Pagamento em configuração"}</button></article>}

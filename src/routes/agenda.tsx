...
import { ArrowLeft, Bot, BriefcaseBusiness, CalendarDays, Clock3, Pencil, Plus, Save, Trash2, UsersRound, X } from "lucide-react";
...
function AgendaPage() {
  ...
  async function loadAppointments() {
    ...
    const { data } = await supabase.from("appointments").select("id,client_id,service_id,professional_id,starts_at,ends_at,status,notes,clients(name),services(name),professionals(name)").eq("organization_id", organizationId).gte("starts_at", start.toISOString()).lte("starts_at", end.toISOString()).order("starts_at");
  ...
  return <main className="min-h-screen bg-[#f7f9f8] text-slate-900">
    ...
    <div className="mx-auto max-w-7xl p-5 sm:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-emerald-600">Agenda</p><h1 className="mt-1 font-[Sora] text-3xl font-bold">Organize seus atendimentos.</h1><p className="mt-2 text-sm text-slate-500">Crie, remarque, atualize e cancele atendimentos com proteção contra conflito.</p></div><button onClick={openCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-600"><Plus className="h-4 w-4" />Novo agendamento</button></div>
      <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-[Sora] text-lg font-semibold">Agenda do dia</h2><p className="text-sm text-slate-500">{appointments.length} atendimento(s) encontrado(s).</p></div><input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="h-11 rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-emerald-500" /></div>
        <div className="mt-5 space-y-3">{appointments.length ? appointments.map((item) => <article key={item.id} className={`grid gap-4 rounded-2xl border p-4 sm:grid-cols-[90px_1fr_auto] sm:items-center ${item.status === "cancelado" ? "border-red-100 bg-red-50/30 opacity-70" : "border-slate-100 bg-slate-50"}`}><div><p className="text-lg font-bold text-slate-950">{new Date(item.starts_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p><p className="text-xs text-slate-400">até {new Date(item.ends_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p></div><div><p className="font-semibold">{item.clients?.name ?? "Cliente"}</p><p className="mt-1 text-sm text-slate-500">{item.services?.name ?? "Serviço"} • {item.professionals?.name ?? "Profissional"}</p>{item.notes && <p className="mt-1 text-xs text-slate-400">{item.notes}</p>}</div><div className="flex items-center gap-2"><span className={`rounded-full px-3 py-1.5 text-xs font-bold capitalize ${statusClass(item.status)}`}>{item.status.replace("_", " ")}</span><button onClick={() => openEdit(item)} className="rounded-xl border border-slate-200 bg-white p-2 text-slate-500 hover:text-slate-950" aria-label="Editar"><Pencil className="h-4 w-4" /></button>{item.status !== "cancelado" && <button onClick={() => cancelAppointment(item)} className="rounded-xl border border-red-100 bg-white p-2 text-red-500 hover:bg-red-50" aria-label="Cancelar"><Trash2 className="h-4 w-4" /></button>}</div></article>) : <div className="py-14 text-center"><CalendarDays className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-600">Nenhum agendamento neste dia.</p></div>}</div>
      </section>
    </div>
